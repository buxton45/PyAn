...
class OutagePredictor:
    ...
    def copy_constructor(
        ...
        self.td_left                   = OutagePredictor.general_copy(outg_pred.td_left)
        self.td_right                  = OutagePredictor.general_copy(outg_pred.td_right)
        self.set_date_range()
        #--------------------------------------------------
    ...
    def set_date_range(
        ...
        """
        #-------------------------
        if not Utilities.is_datetime(obj=self.prediction_date, strict=False):
            self.prediction_date = pd.to_datetime(self.prediction_date)
        if not Utilities.is_timedelta(self.td_left):
            self.td_left         = pd.to_timedelta(self.td_left)
        if not Utilities.is_timedelta(self.td_right):
            self.td_right        = pd.to_timedelta(self.td_right)
        #-------------------------
        self.date_range = [
            (self.prediction_date + self.td_left).date(), 
            (self.prediction_date + self.td_right).date()
        ]
    ...
    def get_date_range_for_prediction(
        ...
        """
        #-------------------------
        if not Utilities.is_datetime(obj=prediction_date, strict=False):
        ...
        #-------------------------
        date_range = [
            (prediction_date + td_left).date(), 
            (prediction_date + td_right).date()
        ]
        #-------------------------
        return date_range
    ...
    def build_raw_data_general(
        ...
        date_ranges = OutagePredictor.get_date_ranges_for_predictions(
            ...
            td_left           = self.td_left, 
            td_right          = self.td_right
        )
        #-----
    ...
    def merge_rcpx_with_eemsp(
        ...
        eemsp_df_i = eemsp_df[
            (eemsp_df[eemsp_install_dt_col]                         <= prediction_date + td_left) & 
            (eemsp_df[eemsp_removal_dt_col].fillna(pd.Timestamp.max) > prediction_date + td_right)
        ]
