...
class KMeansWeightedConstrained(KMeansConstrained):
    """
    K-Means clustering with minimum and maximum cluster size constraints, where size of cluster is determined by weights.
    This was built specificially so I can group similar zip codes (by population, location, etc.) with limits set on the population of each group.
    Based on k_means_constrained.KMeansConstrained (https://github.com/joshlk/k-means-constrained) and checked with the results of the R-code
      contained here: https://rpubs.com/tgarctg/WCK.
    In the case of weights=None, this should reduce to k_means_constrained.KMeansConstrained.
    See publication "Constrained K-Means Clustering" by P.S. Bradley et. al. from Microsoft for the original derivation and motivation behind the
      development of k_means_constrained.KMeansConstrained
      
    !!!!!!!!!! WARNING !!!!!!!!!!
    I have a very surface level understanding of how the constrained k-means can be translated into a Minimum Cost Flow (MCF) linear optimization problem.
    From this basic understanding, I tweaked what I thought was appropriate in code to achieve my desired functionality, and checked with one simple case.
    So.....not scrutinized.  
    ...
    def fit(self, X, weights=None):
        ...
        self.cluster_centers_, self.labels_, self.inertia_, self.n_iter_ = \
            KMeansWeightedConstrained.k_means_constrained(
                X             = X, 
    ...
    #----------------------------------------------------------------------------------------------------
    def predict(self, X, size_min='init', size_max='init', weights=None):
        """
        ...
        if size_max is None:
            if weights is None:
                ...
                size_max = weights.sum()

        # Check size min and max
        KMeansWeightedConstrained.check_size_min_max(
            size_min   = size_min, 
            size_max   = size_max, 
            n_samples  = n_samples, 
            n_clusters = n_clusters, 
        ...
        labels, inertia = KMeansWeightedConstrained.labels_constrained(
            X         = X, 
    ...
    def fit_predict(self, X, weights=None):
        ...
        labels : array, shape [n_samples,]
            Index of the cluster each sample belongs to.
        """
        return self.fit(X=X, weights=weights).labels_
    ...
    #----------------------------------------------------------------------------------------------------
    @staticmethod
    def minimum_cost_flow_problem_graph(
        X, 
        C, 
        D, 
        size_min, 
        ...
        cap_non = weights.sum() # The total supply and therefore wont restrict flow
        #-----
        capacities_X_C_dummy = cartesian([weights, C_dummy_ix])
        capacities_X_C_dummy = [x[0] for x in capacities_X_C_dummy]
        #-----
        ...
        supplies_art = -1 * (weights.sum() - n_C * size_min)  # Demand node
    ...
    def labels_constrained(
        ...
        edges, costs, capacities, supplies, n_C, n_X = KMeansWeightedConstrained.minimum_cost_flow_problem_graph(
            X        = X, 
            C        = C, 
            D        = D, 
            size_min = size_min, 
        ...
        labels = KMeansWeightedConstrained.solve_min_cost_flow_graph(
            edges      = edges, 
            costs      = costs, 
    ...
    def check_size_min_max(
        ...
        #--------------------------------------------------
            ...
            if len(weights) != n_samples:
                raise ValueError("The length of supplied weights must equal n_samples")
            #-------------------------
            # For this case, size_min/_max must be greater than zero and less than the sum of the weights.
            #    Note: For the case of all unit weights, this reduces to n_samples
            #-----
            weights_sum = weights.sum()
            #-----
            if not(
                (size_min >= 0) and (size_min <= weights_sum) and
                (size_max >= 0) and (size_max <= weights_sum)
            ):
                raise ValueError("size_min and size_max must be a positive number smaller "
                                 "than the sum of the weights or `None`")
            #-------------------------
            if size_min * n_clusters > weights_sum:
                raise ValueError("The product of size_min and n_clusters cannot exceed the sum of the weights")
            if size_max * n_clusters < weights_sum:
                raise ValueError("The product of size_max and n_clusters must be larger than or equal the sum of the weights")
    ...
    def kmeans_constrained_single(
        ...
        for i in range(max_iter):
            ...
            labels, inertia = \
                KMeansWeightedConstrained.labels_constrained(
                    X         = X, 
                    centers   = centers, 
                    size_min  = size_min, 
                    size_max  = size_max, 
    ...
    def k_means_constrained(
        ...
        if n_jobs == 1:
            ...
            for it in range(n_init):
                # run a k-means once
                labels, inertia, centers, n_iter_ = KMeansWeightedConstrained.kmeans_constrained_single(
                    X               = X, 
                    n_clusters      = n_clusters,
                    size_min        = size_min, 
                    size_max        = size_max, 
            ...
            results = Parallel(n_jobs=n_jobs, verbose=0)(
                delayed(KMeansWeightedConstrained.kmeans_constrained_single)(
                    X               = X, 
                    n_clusters      = n_clusters,
                    size_min        = size_min, 
                    size_max        = size_max, 
                    ...
                    tol             = tol,
                    x_squared_norms = x_squared_norms,
                    random_state    = seed # Change seed to ensure variety
                )
                for seed in seeds)
