...
class OutageDAQ:
    ...
    def get_summary_dict(
        ...
        if self.collect_evs_sum_vw:
            ...
            top_key = 'end_events'
        #--------------------------------------------------
        summary_dict[top_key]                           = dict()
        #--------------------------------------------------
        summary_dict[top_key]['run_date']               = self.run_date
        #-------------------------
        summary_dict[top_key]['date_0']                 = self.date_0.strftime('%Y-%m-%d %H:%M:%S')
        summary_dict[top_key]['date_1']                 = self.date_1.strftime('%Y-%m-%d %H:%M:%S')
        #-------------------------
        summary_dict[top_key]['collect_evs_sum_vw']     = self.collect_evs_sum_vw
        #-------------------------
        summary_dict[top_key]['td_left']                = self.td_left.total_seconds()
        summary_dict[top_key]['td_right']               = self.td_right.total_seconds()
        #-------------------------
        summary_dict[top_key]['states']                 = self.states
        summary_dict[top_key]['opcos']                  = self.opcos
        summary_dict[top_key]['cities']                 = self.cities
        #-------------------------
        summary_dict[top_key]['single_zip_xfmrs_only']  = self.single_zip_xfmrs_only
        #-------------------------
        summary_dict[top_key]['run_date_subdir_appndx'] = self.run_date_subdir_appndx
        summary_dict[top_key]['dates_subdir_appndx']    = self.dates_subdir_appndx
        summary_dict[top_key]['save_end_events']        = self.save_end_events
        summary_dict[top_key]['save_dfs_to_file']       = self.save_dfs_to_file
        summary_dict[top_key]['read_dfs_from_file']     = self.read_dfs_from_file
        summary_dict[top_key]['run_using_slim']         = self.run_using_slim
        #-------------------------
        summary_dict[top_key]['save_dir_base']          = self.save_dir_base
        summary_dict[top_key]['end_events_save_args']   = self.end_events_save_args
        #--------------------------------------------------
        return summary_dict
    ...
    def read_summary_dict(
        ...
        """
        #--------------------------------------------------
        assert(collect_evs_sum_vw is None or Utilities.is_object_one_of_types(collect_evs_sum_vw, [str, bool]))
        #-----
        if collect_evs_sum_vw is None:
            collect_evs_sum_vw = 'any'
        #-----
        if isinstance(collect_evs_sum_vw, str):
            assert(collect_evs_sum_vw in ['any', 'all'])
        #--------------------------------------------------
        ...
        tmp_f.close()
        #--------------------------------------------------
        if collect_evs_sum_vw == 'all':
            return summary_dict
        #--------------------------------------------------
        if collect_evs_sum_vw == 'any':
            return summary_dict[list(summary_dict.keys())[0]]
        #--------------------------------------------------
        assert(isinstance(collect_evs_sum_vw, bool))
        # Grad top-level keys
        top_keys = list(summary_dict.keys())
        #-------------------------
        if collect_evs_sum_vw==True:
            assert('evs_sum_vw' in top_keys)
            return summary_dict['evs_sum_vw']
        else:
            assert('end_events' in top_keys)
            return summary_dict['end_events']
        #--------------------------------------------------
        assert(0)
...
class OutageDAQOutg(OutageDAQ):
    ...
    def get_summary_dict(
        ...
        if self.collect_evs_sum_vw:
            ...
            top_key = 'end_events'
        #-----
        assert(top_key in summary_dict.keys())
        #--------------------------------------------------
        summary_dict[top_key]['dataset']    = self.__data_info.dataset
        summary_dict[top_key]['rec_nb_col'] = 'outg_rec_nb'
        #--------------------------------------------------
        return summary_dict
    ...
    def save_summary_dict(
        ...
        #-------------------------
        if os.path.exists(os.path.join(self.save_dir_base, 'summary_dict.json')):
            preex_summary_dict = OutageDAQ.read_summary_dict(
                save_dir_base      = self.save_dir_base, 
                summary_dict_fname = 'summary_dict.json', 
                collect_evs_sum_vw = 'all'
            )
            # If overlapping keys exist between preex_summary_dict and summary_dict (e.g., if evs_sum_vw already
            #   exists in preex_summary_dict and we are running DAQ with self.collect_evs_sum_vw==True)
            #   those in summary_dict will be kept.
            # If no overlapping keys, the two dicts will be combined
            summary_dict = preex_summary_dict|summary_dict
        #-------------------------
    ...
    def collect_events(
        ...
        if exit_status:
            ...
            time_infos_df = OutageDAQ.build_baseline_time_infos_df(
                ...
                min_req                 = True, 
                collect_evs_sum_vw      = self.collect_evs_sum_vw, 
                summary_dict_fname      = 'summary_dict.json', 
                alias                   = 'mapping_table', 
                include_summary_paths   = False, 
                consolidate             = False, 
            ...
            if self.collect_evs_sum_vw:
                time_infos_df.to_pickle(os.path.join(self.save_dir_base, 'time_infos_df_evs_sum_vw.pkl'))
            else:
                time_infos_df.to_pickle(os.path.join(self.save_dir_base, 'time_infos_df_end_events.pkl'))
...
class OutageDAQOtBL(OutageDAQ):
    ...
    def get_summary_dict(
        ...
        #--------------------------------------------------
        summary_dict[top_key]['dataset']                = self.__data_info.dataset
        summary_dict[top_key]['rec_nb_col']             = 'no_outg_rec_nb'
        #-------------------------
        summary_dict[top_key]['groupby_col']            = self.groupby_col
        #-------------------------
        summary_dict[top_key]['buffer_time_left']       = self.buffer_time_left.total_seconds()
        summary_dict[top_key]['buffer_time_rght']       = self.buffer_time_rght.total_seconds()
        #-------------------------
        summary_dict[top_key]['pd_selection_stategy']   = self.pd_selection_stategy
        summary_dict[top_key]['search_window_strategy'] = self.search_window_strategy
        #-------------------------
        summary_dict[top_key]['min_window_width']       = self.min_window_width.total_seconds()
        #--------------------------------------------------
        return summary_dict
...
class OutageDAQPrBL(OutageDAQ):
    ...
    def get_summary_dict(
        ...
        summary_dict[top_key]['dataset']                 = self.__data_info.dataset
        summary_dict[top_key]['rec_nb_col']              = 'no_outg_rec_nb'
        summary_dict[top_key]['return_window_strategy']  = self.return_window_strategy
        summary_dict[top_key]['trsf_pole_nbs_to_ignore'] = self.trsf_pole_nbs_to_ignore
        #--------------------------------------------------
        return summary_dict
