...
class OutageModeler:
        ...
        **kwargs
        ...
        Acceptable kwargs:
            ...
            At the time of writing this documentation
                Acceptable mecpx_build_kwargs = list(OutageModeler.get_default_mecpx_build_info_dict().keys()) + OutageModeler.get_acceptable_generic_mecpx_build_args()
                    'acq_run_date',
                    'acq_run_date_otbl',
                    'acq_run_date_outg',
                    'acq_run_date_prbl',
                    'acq_run_date_subdir_appndx', 
                    'acq_run_date_subdir_appndx_otbl'', 
                    'acq_run_date_subdir_appndx_outg', 
                    'acq_run_date_subdir_appndx_prbl', 
                    'barplot_kwargs_shared_otbl',
                    'barplot_kwargs_shared_outg',
                    'barplot_kwargs_shared_prbl',
                    'coll_label_otbl',
                    'coll_label_outg',
                    'coll_label_prbl',
                    'cpx_dfs_name',
                    ...
                    'cpx_dfs_name_prbl',
                    'data_date_ranges',
                    'data_date_ranges_otbl',
                    'data_date_ranges_outg',
                    'data_date_ranges_prbl',
                    'data_dir_base',
                    'data_dir_base_otbl',
                    'data_dir_base_outg',
                    'data_dir_base_prbl',
                    ...
                    'include_power_down_minus_up',
                    'max_total_counts',
                    'mecpo_idx_for_ordering',
                    'mecpx_an_keys',
                    ...
                    'pu_col',
                    'rec_nb_idfr',
                    'regex_to_remove_ignore_case',
                    'regex_to_remove_patterns',
                    'std_dict_grp_by_cols_bsln',
                    'std_dict_grp_by_cols_otbl',
                    'std_dict_grp_by_cols_outg',
                    'std_dict_grp_by_cols_prbl',
                    'trsf_pole_nb_idfr'
                ...
                Acceptable model_args = list(OutageModeler.get_default_model_args_dict().keys())
                    'addtnl_bsln_frac',
                    'an_keys',
                    'an_keys_to_drop',
                    'combine_others',
                    ...
                    'eemsp_enc',
                    'eemsp_mult_strategy',
                    'get_train_test_by_date',
                    'gnrl_slicer',
                    'include_month',
                    'include_total_counts_features',
                    'is_norm',
                    'merge_eemsp',
                    'min_pct_target_1',
                    'n_top_reasons_to_inclue',
                    'outgs_slicer',
                    'pca',
                    'pca_n_components',
                    'random_state',
                    ...
                    'remove_scheduled_outages',
                    'run_PCA',
                    'run_scaler',
                    'scaler',
                    'split_train_test_by_outg',
                    'test_size',
                    'trsf_pole_nbs_to_remove',
                    'val_size'
        """
        #---------------------------------------------------------------------------
        ...
        if len(mecpx_kwargs)>0:
            self.set_mecpx_build_args(
                distribute_to_OMIs = False, 
                extend_any_lists   = extend_lists_in_mecpx_kwargs, 
                **mecpx_kwargs
            )

        #---------------------------------------------------------------------------
        # Initialize the OMI objects
        #   OMI = Outage Modeler Input
        #-----
        # outg  = OUTaGe == signal data 
        # otbl  = Outage Transformer BaseLine
        # prbl  = PRistine BaseLine
        self.__omi_outg  = OMInput(
            dataset                    = 'outg', 
            ...
            init_dfs_to                = init_dfs_to, 
        )
        #-----
        self.__omi_otbl  = OMInput(
            dataset                    = 'otbl', 
        ...
        #-------------------------
        self.distribute_mecpx_build_args_to_OMIs(extend_any_lists = extend_lists_in_mecpx_kwargs)
    ...
    @property
    def omi_outg(self):
        return self.__omi_outg
    @property
    def omi_otbl(self):
        return self.__omi_otbl

    @property
    def omi_prbl(self):
        return self.__omi_prbl
    ...
    #-------------------------
    @property
    def mecpx_coll_outg(self):
        return self.omi_outg.mecpx_coll
    @property
    def mecpx_coll_otbl(self):
        return self.omi_otbl.mecpx_coll
    @property
    def mecpx_coll_prbl(self):
        if self.include_prbl:
            return self.omi_otbl.mecpx_coll
        else:
            return None
    ...
    def copy_constructor(
        ...
        assert(isinstance(outg_mdlr, OutageModeler))
        #--------------------------------------------------
        self.init_dfs_to                     = Utilities.general_copy(outg_mdlr.init_dfs_to)
        #--------------------------------------------------
        self.__omi_outg                      = Utilities.general_copy(outg_mdlr.__omi_outg)
        self.__omi_otbl                      = Utilities.general_copy(outg_mdlr.__omi_otbl)
        self.__omi_prbl                      = Utilities.general_copy(outg_mdlr.__omi_prbl)
        ...
        #-------------------------
        self.eemsp_df                        = Utilities.general_copy(outg_mdlr.eemsp_df)
        self.eemsp_reduce1_df                = Utilities.general_copy(outg_mdlr.eemsp_reduce1_df)
        self.eemsp_df_info_dict              = Utilities.general_copy(outg_mdlr.eemsp_df_info_dict)
        #-------------------------
        self.dovs_df                         = Utilities.general_copy(outg_mdlr.dovs_df)
        #-------------------------
        self.target_col                      = Utilities.general_copy(outg_mdlr.target_col)
        self.from_outg_col                   = Utilities.general_copy(outg_mdlr.from_outg_col)
        #-------------------------
        self.__can_model                     = Utilities.general_copy(outg_mdlr.__can_model)
        #-------------------------
        self.__df_train                      = Utilities.general_copy(outg_mdlr.__df_train)
        self.__df_test                       = Utilities.general_copy(outg_mdlr.__df_test)
        self.__df_holdout                    = Utilities.general_copy(outg_mdlr.__df_holdout)
        self.__df_target                     = Utilities.general_copy(outg_mdlr.__df_target)
        #-------------------------
        self.__X_train                       = Utilities.general_copy(outg_mdlr.__X_train)
        self.__y_train                       = Utilities.general_copy(outg_mdlr.__y_train)
        self.__y_train_pred                  = Utilities.general_copy(outg_mdlr.__y_train_pred)
        #-----
        self.__X_test                        = Utilities.general_copy(outg_mdlr.__X_test)
        self.__y_test                        = Utilities.general_copy(outg_mdlr.__y_test)
        self.__y_test_pred                   = Utilities.general_copy(outg_mdlr.__y_test_pred)
        #-----
        self.__X_holdout                     = Utilities.general_copy(outg_mdlr.__X_holdout)
        self.__y_holdout                     = Utilities.general_copy(outg_mdlr.__y_holdout)
        self.__y_holdout_pred                = Utilities.general_copy(outg_mdlr.__y_holdout_pred)
        #--------------------------------------------------
        self.__model_args_locked             = Utilities.general_copy(outg_mdlr.__model_args_locked)
        #--------------------------------------------------
        self.__an_keys                       = Utilities.general_copy(outg_mdlr.__an_keys)
        #-------------------------
        self.__random_state                  = Utilities.general_copy(outg_mdlr.__random_state)
        #-------------------------
        self.__n_top_reasons_to_inclue       = Utilities.general_copy(outg_mdlr.__n_top_reasons_to_inclue)
        self.__combine_others                = Utilities.general_copy(outg_mdlr.__combine_others)
        #-------------------------
        self.__merge_eemsp                   = Utilities.general_copy(outg_mdlr.__merge_eemsp)
        self.__eemsp_mult_strategy           = Utilities.general_copy(outg_mdlr.__eemsp_mult_strategy)
        self.__eemsp_enc                     = Utilities.general_copy(outg_mdlr.__eemsp_enc)
        #-------------------------
        self.__include_month                 = Utilities.general_copy(outg_mdlr.__include_month)
        #-------------------------
        self.__an_keys_to_drop               = Utilities.general_copy(outg_mdlr.__an_keys_to_drop)
        #-------------------------
        self.__date_0_train                  = Utilities.general_copy(outg_mdlr.__date_0_train)
        self.__date_1_train                  = Utilities.general_copy(outg_mdlr.__date_1_train)
        #-----
        self.__date_0_test                   = Utilities.general_copy(outg_mdlr.__date_0_test)
        self.__date_1_test                   = Utilities.general_copy(outg_mdlr.__date_1_test)
        #-----
        self.__date_0_holdout                = Utilities.general_copy(outg_mdlr.__date_0_holdout)
        self.__date_1_holdout                = Utilities.general_copy(outg_mdlr.__date_1_holdout)
        #-------------------------
        self.__test_size                     = Utilities.general_copy(outg_mdlr.__test_size)
        self.__get_train_test_by_date        = Utilities.general_copy(outg_mdlr.__get_train_test_by_date)
        self.__split_train_test_by_outg      = Utilities.general_copy(outg_mdlr.__split_train_test_by_outg)
        self.__addtnl_bsln_frac              = Utilities.general_copy(outg_mdlr.__addtnl_bsln_frac)
        #-------------------------
        self.__create_validation_set         = Utilities.general_copy(outg_mdlr.__create_validation_set)
        self.__val_size                      = Utilities.general_copy(outg_mdlr.__val_size)
        #-------------------------
        self.__run_scaler                    = Utilities.general_copy(outg_mdlr.__run_scaler)
        self.__scaler                        = Utilities.general_copy(outg_mdlr.__scaler)
        #-------------------------
        self.__run_PCA                       = Utilities.general_copy(outg_mdlr.__run_PCA)
        self.__pca_n_components              = Utilities.general_copy(outg_mdlr.__pca_n_components)
        self.__pca                           = Utilities.general_copy(outg_mdlr.__pca)
        #-------------------------
        self.__outgs_slicer                  = Utilities.general_copy(outg_mdlr.__outgs_slicer)
        self.__gnrl_slicer                   = Utilities.general_copy(outg_mdlr.__gnrl_slicer)
        #-------------------------
        self.__remove_others_from_outages    = Utilities.general_copy(outg_mdlr.__remove_others_from_outages)
        #-------------------------
        self.__min_pct_target_1              = Utilities.general_copy(outg_mdlr.__min_pct_target_1)
        #-------------------------
        self.__reduce_train_size             = Utilities.general_copy(outg_mdlr.__reduce_train_size)
        self.__red_test_size                 = Utilities.general_copy(outg_mdlr.__red_test_size)
        #-------------------------
        self.__is_norm                       = Utilities.general_copy(outg_mdlr.__is_norm)
        self.__include_total_counts_features = Utilities.general_copy(outg_mdlr.__include_total_counts_features)
        self.__remove_scheduled_outages      = Utilities.general_copy(outg_mdlr.__remove_scheduled_outages)
        self.__trsf_pole_nbs_to_remove       = Utilities.general_copy(outg_mdlr.__trsf_pole_nbs_to_remove)
        #-------------------------
        self.__model_clf                     = Utilities.general_copy(outg_mdlr.__model_clf)
        #--------------------------------------------------
        self.save_base_dir                   = Utilities.general_copy(outg_mdlr.save_base_dir)
        self.save_sub_dir                    = Utilities.general_copy(outg_mdlr.save_sub_dir)
        #--------------------------------------------------
        self.__save_data                     = Utilities.general_copy(outg_mdlr.__save_data)
        self.__save_model                    = Utilities.general_copy(outg_mdlr.__save_model)
        #-------------------------
        self.__saved_mecpo_colls             = Utilities.general_copy(outg_mdlr.__saved_mecpo_colls)
        self.__saved_merged_dfs_0            = Utilities.general_copy(outg_mdlr.__saved_merged_dfs_0)
        self.__saved_merged_dfs              = Utilities.general_copy(outg_mdlr.__saved_merged_dfs)
        self.__saved_counts_series_0         = Utilities.general_copy(outg_mdlr.__saved_counts_series_0)
        self.__saved_counts_series           = Utilities.general_copy(outg_mdlr.__saved_counts_series)
        self.__saved_mecpx_build_info_dict   = Utilities.general_copy(outg_mdlr.__saved_mecpx_build_info_dict)
        #-------------------------
        self.__saved_time_infos_df           = Utilities.general_copy(outg_mdlr.__saved_time_infos_df)
        self.__saved_eemsp                   = Utilities.general_copy(outg_mdlr.__saved_eemsp)
        self.__saved_eemsp_enc               = Utilities.general_copy(outg_mdlr.__saved_eemsp_enc)
        self.__saved_data_structure_df       = Utilities.general_copy(outg_mdlr.__saved_data_structure_df)
        self.__saved_scaler                  = Utilities.general_copy(outg_mdlr.__saved_scaler)
        self.__saved_pca                     = Utilities.general_copy(outg_mdlr.__saved_pca)
        self.__saved_summary_dict            = Utilities.general_copy(outg_mdlr.__saved_summary_dict)
        self.__saved_model_clf               = Utilities.general_copy(outg_mdlr.__saved_model_clf)
    ...
    def are_data_normalized(self):
        ...
        #-------------------------
        is_norm_list = [
            self.omi_outg.are_data_normalized(), 
            self.omi_otbl.are_data_normalized(), 
        ]
        if self.include_prbl:
            self.omi_prbl.are_data_normalized()
        #-------------------------
        # Make sure all agree...
    ...
    def self_check_mecpx_build_info_dict(self): 
        r"""
        """
        return OutageModeler.check_mecpx_build_info_dict(self.mecpx_build_info_dict)
    ...
    def get_default_mecpx_build_info_dict():
        ...
        mecpx_build_info_dict['acq_run_date_subdir_appndx_outg'] = None
        ...
        mecpx_build_info_dict['acq_run_date_subdir_appndx_otbl'] = None
        ...
        mecpx_build_info_dict['acq_run_date_subdir_appndx_prbl'] = None
    ...
    #---------------------------------------------------------------------------
    def distribute_mecpx_build_args_to_OMIs(
        self, 
        extend_any_lists = False, 
    ):
        r"""
        Distribute the arguments contained in self.mecpx_build_info_dict to the various OMIs (OMInput objects)
    
        NOTE: Any generic keys should have already been caught by OutageModeler.set_mecpx_build_args, but methods
                included here in case user runs this without the former
        """
        #--------------------------------------------------
        keys_outg = [x for x in self.mecpx_build_info_dict.keys() if x.endswith('_outg')]
        keys_otbl = [x for x in self.mecpx_build_info_dict.keys() if x.endswith('_otbl')]
        keys_prbl = [x for x in self.mecpx_build_info_dict.keys() if x.endswith('_prbl')]
        #-------------------------
        # Can't link all three in joint intersections, as this only guarantees all three don't share,
        #   but two could still share, which we don't want.  Therefore, need pairwise
        assert(set(keys_outg).intersection(set(keys_otbl))==set())
        assert(set(keys_outg).intersection(set(keys_prbl))==set())
        assert(set(keys_otbl).intersection(set(keys_prbl))==set())
        #-------------------------
        keys_othr = [x for x in self.mecpx_build_info_dict.keys() if x not in keys_outg+keys_otbl+keys_prbl]
        #-------------------------
        mecpx_bid_outg = {k[:-5] : v for k,v in self.mecpx_build_info_dict.items() if k in keys_outg}
        mecpx_bid_otbl = {k[:-5] : v for k,v in self.mecpx_build_info_dict.items() if k in keys_otbl}
        mecpx_bid_prbl = {k[:-5] : v for k,v in self.mecpx_build_info_dict.items() if k in keys_prbl}
        mecpx_bid_othr = {k      : v for k,v in self.mecpx_build_info_dict.items() if k in keys_othr}
    
        #--------------------------------------------------
        generic_kwargs = OutageModeler.get_acceptable_generic_mecpx_build_args()
        for generic_key_i in generic_kwargs:
            if generic_key_i not in mecpx_bid_othr.keys():
                continue
            # Need to remove generic_key_i from mecpx_bid_othr and update the specific versions (outg, otbl, prbl), BUT ONLY IF NOT ALREADY CONTAINED
            generic_val_i = mecpx_bid_othr.pop(generic_key_i)
            if generic_key_i in ['grp_by_cols_bsln', 'std_dict_grp_by_cols_bsln']:
                mecpx_bid_otbl[generic_key_i[:5]] = mecpx_bid_otbl.get(generic_key_i[:5], generic_val_i)
                mecpx_bid_prbl[generic_key_i[:5]] = mecpx_bid_prbl.get(generic_key_i[:5], generic_val_i)
            else:
                mecpx_bid_outg[generic_key_i] = mecpx_bid_outg.get(generic_key_i, generic_val_i)
                mecpx_bid_otbl[generic_key_i] = mecpx_bid_otbl.get(generic_key_i, generic_val_i)
                mecpx_bid_prbl[generic_key_i] = mecpx_bid_prbl.get(generic_key_i, generic_val_i)
    
        #--------------------------------------------------
        # At this point, everything left in mecpx_bid_othr should be shared by all!
        # Could have been included in generic, I suppose, but separated out for legacy reasons
        for k,v in mecpx_bid_othr.items():
            mecpx_bid_outg[k] = v
            mecpx_bid_otbl[k] = v
            mecpx_bid_prbl[k] = v
    
        #--------------------------------------------------
        self.__omi_outg.set_mecpx_build_args(
            extend_any_lists = extend_any_lists, 
            **mecpx_bid_outg
        )
        #-----
        self.__omi_otbl.set_mecpx_build_args(
            extend_any_lists = extend_any_lists, 
            **mecpx_bid_otbl
        )
        #-----
        if self.include_prbl:
            self.__omi_prbl.set_mecpx_build_args(
                extend_any_lists = extend_any_lists, 
                **mecpx_bid_prbl
            )
    ...
    def set_mecpx_build_args(
        ...
        **kwargs
        ...
        #----------------------------------------------------------------------------------------------------
        # Distribute the MECPX build arguments to the OMInput objects
        if distribute_to_OMIs:
            self.distribute_mecpx_build_args_to_OMIs(extend_any_lists = extend_any_lists)
    ...
    def get_data_base_dirs(
        ...
        #-------------------------
        if dataset   == 'outg':
            base_dirs = self.omi_outg.get_mecpx_base_dirs(assert_found = assert_found)
        elif dataset == 'otbl':
            base_dirs = self.omi_otbl.get_mecpx_base_dirs(assert_found = assert_found)
        elif dataset == 'prbl':
            base_dirs = self.omi_prbl.get_mecpx_base_dirs(assert_found = assert_found)
        else:
            assert(0)
        #-------------------------
        return base_dirs
    ...
    def try_to_determine_and_apply_new_an_keys(self):
        ...
        #--------------------------------------------------
        try:
            old_to_new_keys_dict = OMInput.build_old_to_new_an_keys(old_an_keys=self.mecpx_build_info_dict['mecpx_an_keys'])
        except:
        ...
        if old_to_new_keys_dict is not None:
            ...
            if self.include_prbl:
                ...
                assert(set(self.mecpx_coll_otbl.mecpo_an_keys).symmetric_difference(set(an_keys))==set())
            #-------------------------
            self.__mecpx_build_info_dict['old_to_new_keys_dict'] = old_to_new_keys_dict
            #-----
            assert(set(an_keys).symmetric_difference(set(old_to_new_keys_dict.values()))==set())
            self.__mecpx_build_info_dict['mecpx_an_keys'] = an_keys
    ...
    def load_mecpx_build_info_dict(
        ...
        if isinstance(self.__mecpx_build_info_dict['rec_nb_idfr'], list):
            self.__mecpx_build_info_dict['rec_nb_idfr'] = tuple(self.__mecpx_build_info_dict['rec_nb_idfr'])
        #-----
        if isinstance(self.__mecpx_build_info_dict['trsf_pole_nb_idfr'], list):
            self.__mecpx_build_info_dict['trsf_pole_nb_idfr'] = tuple(self.__mecpx_build_info_dict['trsf_pole_nb_idfr'])
        #-------------------------
        if verbose:
    ...
    def load_mecpx_colls_from_pkls(
        ...
        # NOTE: If it was supplied, it would have been applied in OMInput.static_build_and_combine_mecpo_colls_for_dates calls above
    ...
    def build_mecpx_colls(
        ...
        #----------------------------------------------------------------------------------------------------
        # Make sure self.mecpx_build_info_dict is distributed to OMInput objects
        #----------------------------------------------------------------------------------------------------
        self.distribute_mecpx_build_args_to_OMIs(extend_any_lists = False)
        
        #----------------------------------------------------------------------------------------------------
        # Build the MECPOCollection objects
        #----------------------------------------------------------------------------------------------------
        #----- Outages (_outg) ----------------------------
        self.__omi_outg.build_and_combine_mecpo_colls_for_dates()
        an_keys = self.omi_outg.mecpx_coll.mecpo_an_keys
    
        #----- Outage Transformers Baseline (_otbl) -------
        self.__omi_otbl.build_and_combine_mecpo_colls_for_dates()
        assert(set(self.omi_otblmecpx_coll.mecpo_an_keys).symmetric_difference(set(an_keys))==set())
    
        #----- Pristine Baseline (_prbl) ------------------
        if self.include_prbl:
            self.__omi_prbl.build_and_combine_mecpo_colls_for_dates()
            assert(set(self.omi_prbl.mecpx_coll.mecpo_an_keys).symmetric_difference(set(an_keys))==set())
