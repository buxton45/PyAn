...
class SQLQuery:
    ...
    def cast_sql_select_element_as_timestamp(
        self             , 
        field_desc       , 
        ...
        if datetime_pattern.lower()=='no_cast':
            return
        #-------------------------
        self.sql_select.cast_element_as_timestamp(
    ...
    def add_join(
        ...
        self.sql_from.add_join_to_coll(
            sql_join  = sql_join, 
            idx       = idx, 
            run_check = run_check)
        #-------------------------
        if join_cols_to_add_to_select is not None:
            self.sql_select.add_select_elements(
                field_descs               = join_cols_to_add_to_select, 
                global_table_alias_prefix = sql_join.join_table_alias, 
                idxs                      = None, 
                run_check                 = run_check)
        
    def build_and_add_join(
        self                       , 
        join_type                  , 
        join_table                 , 
        join_table_alias           , 
        ...
        self.sql_from.build_and_add_join_to_coll(
            join_type               = join_type, 
    ...
    def get_sql_statement(
        ...
        #-----
        return_stmnt =  SQLQuery.combine_sql_components(
            select_stmnt         = select_stmnt, 
    ...
    def print(
        ...
        self.print_sql_statement(
            insert_n_tabs_to_each_line = insert_n_tabs_to_each_line, 
            include_alias              = include_alias
        )
        
    @staticmethod
    def combine_sql_components(
        select_stmnt         , 
