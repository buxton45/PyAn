...
class SQLJoin:
    def __init__(self, join_type, join_table, 
                 join_table_alias, orig_table_alias, 
    ...
    def build_list_of_columns_to_join_dicts(list_of_columns_to_join):
        ...
        assert(isinstance(list_of_columns_to_join, list) or isinstance(list_of_columns_to_join, tuple))
        list_of_columns_to_join_dicts = []
        for cols_to_join in list_of_columns_to_join:
            if isinstance(cols_to_join, dict):
                assert('orig' in cols_to_join or 
                ...
                cols_to_join = {'orig':cols_to_join, 
                                'join':cols_to_join, 
                                'needs_prefixes':True}
    ...
    @staticmethod
    def convert_column_to_join_dict_to_str(col_to_join_dict, join_table_alias, orig_table_alias):
    ...
    @staticmethod
    def build_join_statement(join_type, join_table, join_table_alias, orig_table_alias, list_of_columns_to_join):
    ...
    def get_statement_string(self):
        join_stmnt = SQLJoin.build_join_statement(join_type=self.join_type, join_table=self.join_table, 
                                                  join_table_alias=self.join_table_alias, orig_table_alias=self.orig_table_alias, 
...
class SQLJoinCollection():
    ...
    @staticmethod    
    def check_joins_dict(joins_dict, enforce_pass=True):
        ...
        if len(joins_dict)==0:
            return True
        pass_check = (len(joins_dict)==len(np.unique(list(joins_dict.keys()))) and 
    ...
    @staticmethod
    def insert_to_joins_dict_at_idx(statement, joins_dict, idx=None, run_check=False):
    ...
    def add_join_to_coll(self, sql_join, 
        ...
        self.joins_dict = SQLJoinCollection.insert_to_joins_dict_at_idx(sql_join, 
                                                                        self.joins_dict, 
    ...
    def get_statement_string(self, include_leading_whitespace='\t'):
        #---------------
