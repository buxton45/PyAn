...
class DOVSAudit:
    ...
    def identify_overlaps(
        self, 
    #     overlaps_dovs_sql_fcn=DOVSOutages_SQL.build_sql_std_outage, 
        ...
        #---------------------------------------------------------------------------
        # Update self.best_ests_df to include only those without any overlaps
        # NOTE: Can still retrieve original self.best_ests_df through self.best_ests_df_w_keep_info
        self.best_ests_df = self.best_ests_df_w_keep_info[self.best_ests_df_w_keep_info['keep']==True].drop(
        ...
        self.ami_df_i = DOVSAudit.set_removed_due_to_overlap_in_ami_df_i(
            ami_df_i                   = self.ami_df_i, 
            best_ests_df               = self.best_ests_df_w_keep_info, 
    ...
    def handle_audits_with_same_interval_for_PN(
        ...
        #-------------------------
        return_cols = unq_idfr_cols + list(set([t_min_col, t_max_col, open_beg_col, open_end_col]).difference(set(unq_idfr_cols)))
        #-----
        resolved_i = False
        resolved_i_details = 'Unresolved'
        #-----
        to_adjust_a = pd.DataFrame(columns=return_cols+['adjustment'])
        to_adjust_b = pd.DataFrame(columns=return_cols+['adjustment'])
        #---------------------------------------------------------------------------
        ...
        if ovrlps_dovs_a+ovrlps_dovs_b==0:
            ...
            if closeness_a<=closeness_b:
                to_adjust_a = pd.DataFrame(columns=return_cols+['adjustment'])
                #-----
                to_adjust_b = be_df_b[return_cols].reset_index(drop=True)
                to_adjust_b['adjustment'] = 'remove'
            else:
                to_adjust_b = pd.DataFrame(columns=return_cols+['adjustment'])
                #-----
                to_adjust_a = be_df_a[return_cols].reset_index(drop=True)
    ...
    def handle_audits_with_same_two_intervals_for_PN(
        ...
        resolved_i_details = 'Unresolved'
        #-----
        to_adjust_a = pd.DataFrame(columns=return_cols)
        to_adjust_b = pd.DataFrame(columns=return_cols)
        #-------------------------
        ...
        assert(tmp_ovrlp_dovs_col not in be_df_a.columns.tolist() and 
               tmp_ovrlp_dovs_col not in be_df_b.columns.tolist())
        be_df_a[tmp_ovrlp_dovs_col] = be_df_a.apply(
        ...
        #-----
        be_df_b[tmp_ovrlp_dovs_col] = be_df_b.apply(
            lambda x: pd.Interval(x[t_min_col], x[t_max_col]).overlaps(
        ...
        #-------------------------
        # If each be_df_a and be_df_b has one entry with keep==True and one with keep==False, 
        #   and these differ from each other, then ownership can be resolved simply (the entry with keep==True
        #   will be kept for each)
        if(
            be_df_a[keep_col].sum() == 1 and
            be_df_b[keep_col].sum() == 1 and 
            not be_df_a[be_df_a[keep_col]==True][[t_min_col, t_max_col]].reset_index(drop=True).equals(
                be_df_b[be_df_b[keep_col]==True][[t_min_col, t_max_col]].reset_index(drop=True)
            )
        ):
            to_adjust_a = be_df_a[be_df_a[keep_col]==False][return_cols].reset_index(drop=True)
            assert(to_adjust_a.shape[0]==1)
            #-----
            to_adjust_b = be_df_b[be_df_b[keep_col]==False][return_cols].reset_index(drop=True)
            assert(to_adjust_b.shape[0]==1)
            #-----
            resolved_i = True
            resolved_i_details = 'Certain'
            to_adjust_a['adjustment'] = 'remove'
            to_adjust_b['adjustment'] = 'remove'
        #-------------------------
        # If only one entry in be_df_a overlaps with dovs_outg_t_beg_end_a and 
        #    only one entry in be_df_b overlaps with dovs_outg_t_beg_end_b
        # AND these differ from each other, then ownership can be resolved simply
        elif(
            be_df_a[tmp_ovrlp_dovs_col].sum() == 1 and
            be_df_b[tmp_ovrlp_dovs_col].sum() == 1 and 
            not be_df_a[be_df_a[tmp_ovrlp_dovs_col]==True][[t_min_col, t_max_col]].reset_index(drop=True).equals(
                be_df_b[be_df_b[tmp_ovrlp_dovs_col]==True][[t_min_col, t_max_col]].reset_index(drop=True)
            )
        ):
            to_adjust_a = be_df_a[be_df_a[tmp_ovrlp_dovs_col]==False][return_cols].reset_index(drop=True)
            assert(to_adjust_a.shape[0]==1)
            #-----
            to_adjust_b = be_df_b[be_df_b[tmp_ovrlp_dovs_col]==False][return_cols].reset_index(drop=True)
            assert(to_adjust_b.shape[0]==1)
            #-----
            resolved_i = True
            resolved_i_details = 'Certain'
        ...
        #-------------------------
            ...
            #-------------------------
            ovrlps_df = be_df_a[[t_min_col, t_max_col]].copy()
            #-----
            ...
            if ovrlps_df['belongs_to_outg'].isna().sum()>0:
                ...
                tmp_idx_col = Utilities.generate_random_string()
                assert(tmp_idx_col not in be_df_a.columns.tolist())
                assert(tmp_idx_col not in be_df_b.columns.tolist())
                be_df_a[tmp_idx_col] = range(be_df_a.shape[0])
                be_df_b[tmp_idx_col] = range(be_df_b.shape[0])
                #-------------------------
                # Determine which entries in be_df_a will stay with audit_a by merging be_df_a with to_a
                ...
                # Similarly for audit_b
                idxs_to_adjust_a = [i for i in range(be_df_a.shape[0]) if i not in to_a[tmp_idx_col].tolist()]
                idxs_to_adjust_b = [i for i in range(be_df_b.shape[0]) if i not in to_b[tmp_idx_col].tolist()]
                #-------------------------
                # Form to_adjust_a and to_adjust_b
                to_adjust_a = be_df_a.loc[idxs_to_adjust_a][return_cols].reset_index(drop=True)
                to_adjust_b = be_df_b.loc[idxs_to_adjust_b][return_cols].reset_index(drop=True)
                #-------------------------
                # Cleanup
                be_df_a = be_df_a.drop(columns=[tmp_idx_col])
                be_df_b = be_df_b.drop(columns=[tmp_idx_col])
                #-------------------------
                resolved_i = True
                resolved_i_details = 'Uncertain'
                to_adjust_a['adjustment'] = 'remove'
                to_adjust_b['adjustment'] = 'remove'
        #-------------------------
        # Need to remove tmp_ovrlp_dovs_col
        be_df_a = be_df_a.drop(columns=[tmp_ovrlp_dovs_col])
        be_df_b = be_df_b.drop(columns=[tmp_ovrlp_dovs_col])
        #-------------------------
        return dict(
            resolved_i         = resolved_i, 
            resolved_i_details = resolved_i_details, 
    ...
    def implement_adjustments_for_audit(
        ...
        for k,adjstmnt_dict in adjustment_dicts.items():
            #-------------------------
            # Can only adjust those which have been resolved
            if adjstmnt_dict['resolved_i']==False:
                continue
            #-------------------------
    ...
    def resolve_overlapping_audits_for_PN(
        ...
        return_cols = unq_idfr_cols + list(set([t_min_col, t_max_col, open_beg_col, open_end_col]).difference(set(unq_idfr_cols)))
        #--------------------------------------------------
        # If one of the audits does not contain PN_i, or the audits don't overlap for the given PN, return
        if(
            not(
                PN_i in audit_a.best_ests_df_w_keep_info[PN_col].tolist() and 
                PN_i in audit_b.best_ests_df_w_keep_info[PN_col].tolist()
            ) or
            audit_b.outg_rec_nb not in list(set(audit_a.best_ests_df_w_keep_info[overlap_DOVS_col].sum())) or
            audit_a.outg_rec_nb not in list(set(audit_b.best_ests_df_w_keep_info[overlap_DOVS_col].sum()))
        ...
        #--------------------------------------------------
        if pd.Interval(*dovs_outg_t_beg_end_a).overlaps(pd.Interval(*dovs_outg_t_beg_end_b)):
            resolved_i = False
            resolved_i_details = f'DOVS events with outg_rec_nb = {outg_rec_nb_a} and {outg_rec_nb_b} overlap!'
        ...
        if DOVSAudit.did_audits_find_same_interval_for_PN(
            be_df_a   = be_df_a, 
            be_df_b   = be_df_b, 
            t_min_col = t_min_col, 
            t_max_col = t_max_col
    ...
    @staticmethod
    def resolve_overlapping_audits_v2(
        audit_a, 
        audit_b, 
        PNs_to_resolve=None, 
        unq_idfr_cols = ['PN', 'i_outg'], 
